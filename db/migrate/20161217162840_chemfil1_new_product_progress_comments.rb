...
class Chemfil1NewProductProgressComments < ActiveRecord::Migration
  def up
    sql1 = <<-SQL
-- ----------------------------------------------------------------------------
-- Table CHEMFIL1.New_Product_Progress_Data_Comments
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `New_Product_Progress_Data_Comments` (
  `Product Code` VARCHAR(255) NOT NULL,
  `Comments` TEXT NULL,
  UNIQUE INDEX (`Product Code`),
  FOREIGN KEY (`Product Code`) REFERENCES `New_Product_Progress_Data`(`Product Code`) ON DELETE CASCADE
  );
SQL

  sql2 = <<-SQL
INSERT INTO `New_Product_Progress_Data_Comments` (`Product Code`, `Comments`)
SELECT `Product Code`, `Comments` FROM `New_Product_Progress_Data`;
SQL

  sql3 = <<-SQL
ALTER TABLE `New_Product_Progress_Data` DROP COLUMN `Comments`;
SQL

    Chemfil1Migration.execute(sql1)
    Chemfil1Migration.execute(sql2)
    Chemfil1Migration.execute(sql3)
    ...
    # setup new comment etl
    create_trigger("New_Product_Progress_Data_Comments_AFTER_INSERT", :generated => false, :compatibility => 1).
        on("New_Product_Progress_Data_comments").
        after(:insert) do
      destination_db = Rails.application.config.database_configuration()[Rails.env]['database']
      ...
      <<-SQL_ACTIONS
        thisTrigger: BEGIN
          ...
          SET comments = NEW.`Comments`,
              updated_at = NOW()
          WHERE code = NEW.`Product Code`;
        END thisTrigger;
      SQL_ACTIONS
    end

    create_trigger("New_Product_Progress_Data_Comments_AFTER_UPDATE", :generated => false, :compatibility => 1).
        on("New_Product_Progress_Data_comments").
        after(:update) do
      destination_db = Rails.application.config.database_configuration()[Rails.env]['database']
