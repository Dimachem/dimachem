...
ActiveRecord::Schema.define(version: 20160815011715) do
  ...
  # WARNING: generating adapter-specific definition for formulas_progress_steps_after_delete_row_tr due to a mismatch.
  # either there's a bug in hairtrigger or you've messed up your migrations and/or db :-/
  execute(<<-TRIGGERSQL)
CREATE TRIGGER formulas_progress_steps_after_delete_row_tr AFTER DELETE ON `formulas_progress_steps`
FOR EACH ROW
BEGIN
        thisTrigger: BEGIN
...
END
  ...
  # WARNING: generating adapter-specific definition for formulas_progress_steps_after_insert_row_tr due to a mismatch.
  # either there's a bug in hairtrigger or you've messed up your migrations and/or db :-/
  execute(<<-TRIGGERSQL)
CREATE TRIGGER formulas_progress_steps_after_insert_row_tr AFTER INSERT ON `formulas_progress_steps`
FOR EACH ROW
BEGIN
        thisTrigger: BEGIN
          IF @TRIGGER_CHECKS_CHEMFIL1 = FALSE THEN
...
END
  ...
  # WARNING: generating adapter-specific definition for formulas_progress_steps_after_update_row_tr due to a mismatch.
  # either there's a bug in hairtrigger or you've messed up your migrations and/or db :-/
  execute(<<-TRIGGERSQL)
CREATE TRIGGER formulas_progress_steps_after_update_row_tr AFTER UPDATE ON `formulas_progress_steps`
FOR EACH ROW
BEGIN
        thisTrigger: BEGIN
          IF @TRIGGER_CHECKS_CHEMFIL1 = FALSE THEN
